...
public:
   ...
   // writes graph to file ... just for testing!
   void writeGraphToFile(const char* filepath, bool useFormatTgf = false, bool zeroIndexed = false)
   {
      using namespace std;
      using myedge = std::tuple<int64_t, int64_t, float>;
      int64_t nnodes = 0;
      std::ofstream myfile;
      myfile.open(filepath);
      vector<myedge> edges;

      const int num_loops = this->beginRead(false);
      for( int loop_count = 0; loop_count < num_loops; ++loop_count ) {
         EdgeType *edge_data;
         const int edge_data_length = this->read(&edge_data);

         for( int i = 0; i < edge_data_length; ++i ) {
            const int64_t v0 = edge_data[i].v0();
            const int64_t v1 = edge_data[i].v1();

            nnodes = std::max({nnodes, v0, v1});

            if( v0 == v1 )
               continue;

            const int64_t v_min = std::min(v0, v1);
            const int64_t v_max = std::max(v0, v1);

            edges.emplace_back(make_tuple(v_min, v_max, edge_data[i].weight()));
         }
      }
      nnodes++;
      this->endRead();
      ...
      // lexicographic sort
      auto pred = [](myedge e1, myedge e2) {
         return (get<0>(e1) < get<0>(e2) || (get<0>(e1) == get<0>(e2) && get<1>(e1) < get<1>(e2)) || (get<0>(e1) == get<0>(e2) && get<1>(e1) == get<1>(e2) && get<2>(e1) < get<2>(e2)));
      };
      std::sort(edges.begin(), edges.end(), pred);

      auto edgesAreEqual = [](myedge e1, myedge e2) {
      ...
      if( !useFormatTgf ) {
         ...
         for( size_t i = 0; i < edges.size(); i++ )
            if( i == 0 || !edgesAreEqual(edges[i], edges[i - 1]) )
               nedges++;
         myfile << nnodes << " " << nedges << '\n';
      }
      ...
      for( size_t i = 0; i < edges.size(); i++ )
      {
         if( i != 0 && edgesAreEqual(edges[i], edges[i - 1]) )
            continue;
         if( zeroIndexed )
            myfile << get<0>(edges[i]) << " " << get<1>(edges[i]) << "   " << get<2>(edges[i]) << '\n';
         else
            myfile << get<0>(edges[i]) + 1 << " " << get<1>(edges[i]) + 1 << "   " << get<2>(edges[i]) << '\n';
      }
      ...
      std::vector<int> degree_org(nnodes, 0);
      for( size_t i = 0; i < edges.size(); i++ )
      {
         degree_org[get<0>(edges[i])]++;
         degree_org[get<1>(edges[i])]++;

         if( i != 0 && edges[i] == edges[i - 1] )
            continue;

         degree[get<0>(edges[i])]++;
         degree[get<1>(edges[i])]++;
      }

      std::cout << "org degree:" << '\n';

      for( size_t i = 0; i < degree.size(); i++ )
         std::cout <<  i  << ": " << degree_org[i] << '\n';
...
#else
			edge_buffer[edge_index - start_edge].weight_ = mrg_get_float_orig(&new_state);
#endif
		}
#endif
	}
...
#pragma omp for
		for(int64_t edge_index = std::max(start_edge, this->num_initial_edges());
			...
			// todo any difference to below?
	//		edge_buffer[edge_index - start_edge].weight = mrg_get_float_orig(&new_state);
		}
