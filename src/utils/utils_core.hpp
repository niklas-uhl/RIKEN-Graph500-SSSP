...
//-------------------------------------------------------------//
// For type numerical comparison
//-------------------------------------------------------------//

namespace comp {
   const float eps_default = 2e-6;
   const float infinity = 1e6;

   inline bool isNegative(float f, float eps = eps_default) {
      return (f < - eps);
   }

   inline bool isZero(float f, float eps = eps_default) {
      return (std::fabs(f) < eps);
   }

   inline bool isEQ(float f1, float f2, float eps = eps_default) {
      return (std::fabs(f1 - f2) <= eps);
   }

   inline bool isGT(float f1, float f2, float eps = eps_default) {
      return (f1 > f2 + eps);
   }

   inline bool isGE(float f1, float f2, float eps = eps_default) {
      return (f1 >= f2 - eps);
   }

   inline bool isLT(float f1, float f2, float eps = eps_default) {
      return (f1 < f2 - eps);
   }

   inline bool isLE(float f1, float f2, float eps = eps_default) {
      return (f1 <= f2 + eps);
   }
}
...
static_assert(sizeof(float) == sizeof(uint32_t), "non-matching float/uint32 sizes");

//#define USE_CAST_UNION


inline uint32_t castFloatToUInt32(float f) {
#ifdef USE_CAST_UNION
   union { float f; uint32_t uint; } u;
   u.f = f;
   return u.uint;
#else
   uint32_t uint;
   std::memcpy(&uint, &f, sizeof(uint32_t));
   return uint;
#endif
}

inline float castUInt32ToFloat(uint32_t uint) {
#ifdef USE_CAST_UNION
   union { float f; uint32_t uint; } u;
   u.uint = uint;
   return u.f;
#else
   float f;
   std::memcpy(&f, &uint, sizeof(uint32_t));
   return f;
#endif
}
...
// Get most significant bit index; if value = 0, the result is undefined.
