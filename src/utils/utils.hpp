...
#if VERBOSE_MEM_MODE
#define VERBOSE_MEM(s) s
#else
#define VERBOSE_MEM(s)
#endif
...
void* cache_aligned_xcalloc(const size_t size) {
	...
	VERBOSE_MEM(x_allocate_check(p));
	memset(p, 0, size);
	return p;
}
...
static void setup_2dcomm()
{
	...
	if(!success && tofu_6d) {
		...
		char *tmp_env = getenv("PROCS_PER_NODE");
		int procs_per_node = (tmp_env == NULL)? 1 :atoi(tmp_env);
		if(mpi.isMaster()) print_with_prefix("Procs per node =  %d\n", procs_per_node);
		if(procs_per_node <= 0){
		  print_with_prefix("PROCS_PER_NODE is invalid\n");
		  MPI_Abort(MPI_COMM_WORLD, 1);
		}
		rank6d[0] = rank6d[0] * procs_per_node + mpi.rank % procs_per_node;
	...
	if(!success) {
		...
		if(twod_r_str){
			...
			for(twod_r = (int)ceil(sqrt(mpi.size)); twod_r < mpi.size; ++twod_r) {
				twod_c = mpi.size / twod_r;
...
#pragma omp parallel
	...
	WeightedOwnerEdge::uninitialize();
...
#pragma omp parallel for if(comm_size_ > 1000)
	...
	int* get_send_lengths() { return send_counts_; }
	...
	void alltoallv(void* sendbuf, void* recvbuf, MPI_Datatype type, int recvbufsize)
	{
		...
		if(recv_offsets_[comm_size_] > recvbufsize) {
		   std::cout << "buffer alltoallv issue: " <<  recv_offsets_[comm_size_] << " > " << recvbufsize << '\n';
			fprintf(IMD_OUT, "Error: recv_offsets_[comm_size_] > recvbufsize");
...
public:
			...
			int64_t max_size)
	{
		...
		outbuf = static_cast<uint8_t*>(output);
...
#define TO_S(offset, length) (O_TO_S(offset) + L_TO_S(length))
		...
		VERBOSE(print_with_prefix("Move %ld length", out_len - sizeof(PacketIndex) - O_TO_S(pk_head[i+1].offset)));
		for( ; i < num_packet; ++i) {
...
#pragma omp parallel
	...
	assert(memcmp((int8_t*)dst, (int8_t*)src, size) == 0);
}


void clean_mt(void* dst, size_t size) {
#pragma omp parallel
   {
      ...
      int64_t i_start, i_end;
      get_partition<int64_t>(size, num_threads, tid, i_start, i_end);
      memset((int8_t*)dst + i_start, 0, i_end - i_start);
   }
#ifndef NDEBUG
   for( size_t i = 0; i < size; i++ )
      assert(((int8_t*)dst)[i] == 0);
#endif
}
...
#if VERBOSE_MODE
volatile int64_t total_edge_top_down;
volatile int64_t total_edge_bottom_up;
