...
// data to be sent (to right process) and received (from left process) during the bottom-up phase
struct BottomUpSubstepData {
	BottomUpSubstepTag tag;
	void* data;
};

// base bottom-up communication class
...
protected:
	...
	VERBOSE(profiling::TimeKeeper tk_);
	VERBOSE(std::vector<int64_t> compute_time_);
	VERBOSE(std::vector<int64_t> comm_wait_time_);
	...
	int n_buffers_available() {
		return (int)free_list.size();
	}

	void* get_buffer() {
		assert(n_buffers_available() > 0);
	...
	// registers the given buffers internally
	template <typename T>
...
#endif
		VERBOSE(tk_.getSpanAndReset());
		VERBOSE(compute_time_.clear());
		VERBOSE(comm_wait_time_.clear());
	}
};

class MpiBottomUpSubstepComm : public BottomUpSubstepCommBase {
	using super__ = BottomUpSubstepCommBase;
public:
...
protected:
   ...
   CommTargetBase& nodes(int target) override { return nodes_[target]; }
	...
	void next_recv() override {
		next_recv_probe(true);
	}

	// non-blocking send-receive with left and right neighbors
	void send_recv() override {
		VERBOSE(compute_time_.push_back(tk_.getSpanAndReset()));
		next_recv_probe(true);
		VERBOSE(comm_wait_time_.push_back(tk_.getSpanAndReset()));
