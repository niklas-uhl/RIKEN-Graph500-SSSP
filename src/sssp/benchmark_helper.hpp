...
#pragma omp parallel
	for(int64_t i = 0; i < num_iterations; ++i) {
		...
		// NOTE: necessary so that all edges ranges are created
#pragma omp barrier
...
template <typename GraphType>
void find_roots_presolved(GraphType& g, int64_t* bfs_roots, int& num_bfs_roots)
{
   const int64_t local_bitmap_width = g.num_local_verts_ / PRM::NBPE;
   assert(!g.has_edge_bitmap_);
   ...
   find_roots(g, bfs_roots, num_bfs_roots);

   free(g.has_edge_bitmap_); g.has_edge_bitmap_= nullptr;
}
...
// NOTE: hack to suppress unused value warning
static inline void ignore_value (size_t i) { (void) i; }
...
int read_log_file(LogFileFormat* log, int SCALE, int edgefactor, double* bfs_times, double* validate_times, double* edge_counts)
{
	...
	if(logfilename) {
		if(mpi.isMaster()) {
			...
			if(fp != NULL) {
			   ignore_value(fread(log, sizeof(log[0]), 1, fp));
